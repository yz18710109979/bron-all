...
public interface WeEnterpriseTokenClient {
    ...
    // 日历相关
    // 日历添加
    @Request(url = Constants.ENTERPRISE_OA_CALENDAR_ADD,
            type = HttpMethod.POST,
            interceptor = {ForestEnterpriseInterceptor.class})
    ...
    /** 客户管理关系 */
    // 获取配置了客户联系功能的成员列表
    @Request(url = Constants.ENTERPRISE_GET_EXTCONTACT_FOLLOW_USER_LIST,
            type = HttpMethod.GET,
            interceptor = {ForestEnterpriseInterceptor.class}
            ) String getExtContactFollowUserList();

    // 获取指定成员添加的客户列表
    @Request(url = Constants.ENTERPRISE_GET_EXTCONTACT_LIST,
            type = HttpMethod.GET,
            interceptor = {ForestEnterpriseInterceptor.class}) String getExtContactList(String userId);

    // 获取配置过客户群管理的客户群列表。
    @Request(url = Constants.ENTERPRISE_GET_EXTCONTACT_GROUP_CHAT_LIST,
            type = HttpMethod.POST,
    ...
    @Request(url = Constants.ENTERPRISE_LIVING_GET_USER_ALL_LIVING_ID,
            type = HttpMethod.POST,
            interceptor = {ForestEnterpriseInterceptor.class})
    ...
    @Request(url = Constants.ENTERPRISE_LIVING_GET_LIVING_INFO,
            type = HttpMethod.GET,
            interceptor = {ForestEnterpriseInterceptor.class})
    WeLivingResultDto livingGetLivingInfo(String corpId, String permentCode,  String livingid);

    //获取直播观看明细
    @Request(url = Constants.ENTERPRISE_LIVING_GET_WATCH_STATE,
            type = HttpMethod.POST,
            interceptor = {ForestEnterpriseInterceptor.class})
